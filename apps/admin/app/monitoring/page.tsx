...
function groupKey(session: any) {
  return `${session.roomId ?? "tanpa-ruang"}:${session.examPackageId}:${session.startsAt.toISOString()}:${session.endsAt.toISOString()}`;
}
...
export default async function MonitoringPage() {
  const user = await requirePageUser(["EXAM_ADMIN", "SUPER_ADMIN"]);
  ...
  return (
    <AdminShell
      ...
      subtitle="Dashboard admin untuk memantau sesi/ruang, login peserta, status pengerjaan, dan pelanggaran"
