...
const AUTH_COOKIE = "seleksi_admin_session";
...
export async function requireActionUser(allowedRoles: string[]) {
  const user = await getCurrentUser();
  if (!user) throw new Error("Sesi tidak valid. Silakan login kembali.");
  if (!canAccess(user.roles, allowedRoles)) {
    throw new Error("Anda tidak memiliki izin untuk menjalankan tindakan ini.");
  }
  return user;
}
